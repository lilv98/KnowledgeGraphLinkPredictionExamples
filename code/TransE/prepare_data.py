...
class TrainSet(Dataset):
    ...
    def load_text(self):
        ...
        raw_data = pd.read_csv('./openbiolink/train.csv', sep='\t', header=None,
                               names=['head', 'relation', 'tail'],
                               keep_default_na=False, encoding='utf-8')
        test_data = pd.read_csv('./openbiolink/test.csv', sep='\t', header=None,
                        names=['head', 'relation', 'tail'],
                        keep_default_na=False, encoding='utf-8')
        ...
        entity_list = list(set(raw_data['head'].unique()) | set(test_data['head'].unique()) | set(raw_data['tail'].unique()) | set(test_data['tail'].unique()))
        relation_list = list(set(raw_data['relation'].unique()) | set(test_data['relation'].unique()))
        # pdb.set_trace()
...
class TestSet(Dataset):
    ...
    def load_text(self):
        ...
        #                        names=['head', 'relation', 'tail'],
        #                        keep_default_na=False, encoding='utf-8')
        raw_data = pd.read_csv('./openbiolink/test.csv', sep='\t', header=None,
                        names=['head', 'relation', 'tail'],
                        keep_default_na=False, encoding='utf-8')
        raw_data = raw_data.applymap(lambda x: x.strip())
