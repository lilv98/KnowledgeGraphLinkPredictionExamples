...
class Generator(torch.nn.Module):
    def __init__(self, emb_dim):
        ...
        self.model = torch.nn.Sequential(
            *block(self.emb_dim * 3, self.emb_dim),
            *block(self.emb_dim, self.emb_dim)
        )

    def forward(self, condition, z):
        return self.model(torch.cat([condition, z], dim=1))
...
def get_ng_G(X_emb):
    ...
    h_and_r = torch.cat([X_emb[:bs, :50], X_emb[:bs, 50:100]], dim=1)
    t_and_r = torch.cat([X_emb[:bs, 100:], X_emb[:bs, 50:100]], dim=1)
    fake_head = G_head(h_and_r, z_head)
    fake_tail = G_tail(t_and_r, z_tail)
...
def D_loss(pred):
    pred = pred.view(bs, -1)
    margins = pred[:, 0].unsqueeze(-1) - pred[:, 1:] + margin
