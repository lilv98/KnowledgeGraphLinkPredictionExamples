...
def D_loss(pred):
    pred = pred.view(bs, -1)
    loss_P = pos_ratio * (torch.log(pred[:, 0] + 1e-10) - torch.log(1 - pred[:, 0] + 1e-10)).sum()
    margins = pred[:, 0].unsqueeze(-1) - pred[:, 1:]
    margins[margins < margin] = 1
    loss_U = torch.log(margins).sum()
    return -(loss_P + loss_U)

def G_loss(pred):
    pred = pred.view(bs, -1)
    return torch.log(1 - pred[:, 3:] + 1e-10).sum()
